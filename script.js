...
while (!nombreCompleto || nombreCompleto.length < 5) {
    nombreCompleto = prompt("Hola! Ingresa tu nombre por favor");
...
wallet.push(saldo);
alert(`Tu saldo inicial es de: $${saldoInicial.toFixed(2)}`);
console.log(`Tu saldo inicial es de: $${saldoInicial.toFixed(2)}`)
...
function registrarTransaccion(monto) {
    ...
    while (!movimientoReferencia) {
        ...
        if (!movimientoReferencia) {
            alert("Ingresa una referencia, ejemplo: Ahorro semanal o gasto diario")
...
function ingresarSaldo() {
    ...
    console.log(`Tu deposito es de: $${ingresos.toFixed(2)}`)
}
...
function restarSaldo() {
    ...
    while (true) {
        ...
        alert(`Ingresa una cantidad válida, recorda que tu saldo es de: $${saldo.toFixed(2)}`);
    }
    registrarTransaccion(-egresos);

    console.log("Tu retiro es de: $" + egresos.toFixed(2))
}
...
while (ingresarMovimientos) {
    let walletMovimientos = prompt("Para depositos coloca 0, para retiro coloca 1, para salir coloca 2 o B para buscar por referencia")
    switch (walletMovimientos) {
        ...
        case "2":
            ...
            alert(`Gracias ${nombre}! Tu saldo final es de: $${saldo.toFixed(2)}`);

            console.log(`Gracias ${nombre}! Tu saldo final es de: $ ${saldo.toFixed(2)}`);
            break
        case "B":
            buscarReferencia()
            break
